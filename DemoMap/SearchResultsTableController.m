...
- (void)selectStationButtonClicked :(UIButton *)sender {
    Station *selectedStation = self.filteredStations[sender.tag];
    
    ChooseStationsViewController *csvc = [self.storyboard instantiateViewControllerWithIdentifier:@"ChooseStationsViewController"];

    if (self.searchStart) {
        csvc.isSelectingStart = YES;
        csvc.startStation = selectedStation;
    } else {
        csvc.isSelectingStart = NO;
        csvc.endStation = selectedStation;
    }
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    NSLog(@"Push from Search Results");
}
